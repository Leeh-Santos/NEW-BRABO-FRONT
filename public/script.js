...
async function loadContractStats() {
    ...
    try {
        ...
        if (totalFundersElem) {
            totalFundersElem.textContent = (parseInt(totalFunders.toString()) + 50).toString();
        }
...
async function updateLiquidityStatus() {
    ...
    try {
        ...
        const ethPrice = await getEthPriceUSD();
        const batchUsd = parseFloat(batchEth) * ethPrice;
        const minUsd = parseFloat(minEth) * ethPrice;
        ...
        // Update available batch amount USD
        const availableBatchAmountUsdElem = document.getElementById('availableBatchAmountUsd');
        if (availableBatchAmountUsdElem) {
            availableBatchAmountUsdElem.textContent = `$${formatNumber(batchUsd, 2)}`;
        }
        ...
        // Update minimum required USD
        const minLiquidityAmountUsdElem = document.getElementById('minLiquidityAmountUsd');
        if (minLiquidityAmountUsdElem) {
            minLiquidityAmountUsdElem.textContent = `$${formatNumber(minUsd, 2)}`;
        }
        ...
        const liquidityProgressFillElem = document.getElementById('liquidityProgressFill');
        const currentBatchLabelElem = document.getElementById('currentBatchLabel');
        const targetBatchLabelElem = document.getElementById('targetBatchLabel');

        if (progressPercentageElem && liquidityProgressFillElem && currentBatchLabelElem && targetBatchLabelElem) {
            // Calculate percentage (cap at 100%)
            const progressPercentage = Math.min((parseFloat(batchEth) / parseFloat(minEth)) * 100, 100);

            // Update progress percentage text
            progressPercentageElem.textContent = `${progressPercentage.toFixed(1)}%`;

            // Update progress bar width
            liquidityProgressFillElem.style.width = `${progressPercentage}%`;

            // Update current batch label
            currentBatchLabelElem.textContent = `${formatNumber(batchEth, 4)} ETH`;

            // Update target batch label
            targetBatchLabelElem.textContent = `${formatNumber(minEth, 4)} ETH`;
        }
...
function initializeEventListeners() {
    ...
    async function initializeReadOnlyProvider() {
        try {
            // FOR TESTING REMEMBNER TO ADD AVIL LOCAL NODE BACK IN HHTTP127.1.8545
        ...
        console.log('Read-only provider initialized with Anvil local network');
        await loadContractStats();
    } catch (error) {
...
async function initialize() {
    ...
    // ALWAYS initialize read-only provider FIRST to show stats immediately
    // This runs before any wallet connection logic and loads stats for all users
...
function initializePortfolioPage() {
    ...
    // Only run if we're on the portfolio page
    if (!connectPrompt && !portfolioContent) return;
    ...
    // Load portfolio data if connected
    if (state.isConnected) {
        loadPortfolioData();
    }
}
...
async function loadPortfolioData() {
    if (!state.userAddress || !state.fundMeContract || !state.nftContract) {
        console.log('Cannot load portfolio: wallet not connected or contracts not initialized');
        return;
    }
    ...
    try {
        ...
        // Fetch basic data
        const [userFundedWei, userFundedUsdWei, ethPrice] = await Promise.all([
            state.fundMeContract.getHowMuchDudeFunded(state.userAddress),
            state.fundMeContract.getHowMuchDudeFundedInUsd(state.userAddress),
            getEthPriceUSD()
        ]);
        ...
        const portfolioEthFundedElem = document.getElementById('portfolioEthFunded');
        const portfolioUsdFundedElem = document.getElementById('portfolioUsdFunded');

        if (portfolioEthFundedElem) portfolioEthFundedElem.textContent = `${parseFloat(ethFunded).toFixed(6)} ETH`;
        if (portfolioUsdFundedElem) portfolioUsdFundedElem.textContent = `$${usdFunded.toFixed(2)}`;

        // Get BRB balance from wallet
        let brbBalance = 0;
        try {
            const brbTokenAddress = CONTRACT_ADDRESSES.PICA_TOKEN;
            const erc20Abi = ['function balanceOf(address) view returns (uint256)'];
            const brbTokenContract = new ethers.Contract(brbTokenAddress, erc20Abi, state.provider);
            const brbBalanceWei = await brbTokenContract.balanceOf(state.userAddress);
            brbBalance = parseFloat(ethers.formatUnits(brbBalanceWei, 18));

            const portfolioBrbReceivedElem = document.getElementById('portfolioBrbReceived');
            if (portfolioBrbReceivedElem) {
                portfolioBrbReceivedElem.textContent = `${brbBalance.toLocaleString('en-US', {
        ...
        // Calculate BRB value by fetching price from API
        try {
            const portfolioBrbValueElem = document.getElementById('portfolioBrbValue');

            if (portfolioBrbValueElem) {
                // Fetch BRB price from GeckoTerminal
                const POOL_ADDRESS = '0x224978d8428b56119c4C2BcE1d8D64B1Af5FAC29';
                const response = await fetch(`https://api.geckoterminal.com/api/v2/networks/base/pools/${POOL_ADDRESS}`);
                const data = await response.json();
                ...
                if (data && data.data && data.data.attributes) {
                    ...
                    if (priceUsd && priceUsd > 0 && !isNaN(priceUsd)) {
                        const brbValue = brbBalance * priceUsd;
                        portfolioBrbValueElem.textContent = `$${brbValue.toFixed(2)}`;
                    } else {
                        portfolioBrbValueElem.textContent = `$0.00`;
                    }
                } else {
                    portfolioBrbValueElem.textContent = `$0.00`;
                }
        ...
        try {
            nftBalance = await state.nftContract.balanceOf(state.userAddress);
            if (Number(nftBalance) > 0) {
                try {
                    userTier = await state.nftContract.getUserTier(state.userAddress);
                } catch (tierError) {
                    console.log('Could not get user tier:', tierError.message);
                    userTier = 0;
                }
            }
        } catch (nftError) {
            console.log('NFT data not available:', nftError.message);
            nftBalance = 0n;
            userTier = 0;
        }

        // Update NFT Section
        updateNftSection(nftBalance, userTier);

        // Update Tier Progression
        updateTierProgression(usdFunded, userTier);
...
async function updateNftSection(nftBalance, userTier) {
    ...
    if (!nftEmptyState || !nftCard) return;
    ...
    if (hasNft) {
        ...
        try {
            const tokenId = await state.nftContract.getTokenIdByOwner(state.userAddress);
            const tierNames = ['Bronze', 'Silver', 'Gold'];
            const tierBonuses = ['+2%', '+5%', '+10%'];
            const tier = Number(userTier);

            const nftTokenIdElem = document.getElementById('nftTokenId');
            const nftTierElem = document.getElementById('nftTier');
            const nftBonusElem = document.getElementById('nftBonus');
            const nftTierBadge = document.getElementById('nftTierBadge');
            const nftImage = document.getElementById('nftImage');

            if (nftTokenIdElem) nftTokenIdElem.textContent = `Token #${tokenId}`;
            if (nftTierElem) nftTierElem.textContent = tierNames[tier] || 'Bronze';
            if (nftBonusElem) nftBonusElem.textContent = tierBonuses[tier] || '+2%';

            if (nftTierBadge) {
                nftTierBadge.textContent = tierNames[tier] || 'Bronze';
                nftTierBadge.className = 'nft-tier-badge';
                if (tier === 1) nftTierBadge.classList.add('silver');
                if (tier === 2) nftTierBadge.classList.add('gold');
            }
...
function updateTierProgression(usdFunded, userTier) {
    const tier = Number(userTier);
    ...
    const currentTierNameElem = document.getElementById('currentTierName');
    if (currentTierNameElem) currentTierNameElem.textContent = currentTierName;
    ...
    const nextTierNameElem = document.getElementById('nextTierName');
    if (nextTierNameElem) nextTierNameElem.textContent = nextTierName;
    ...
    const tierProgressFillElem = document.getElementById('tierProgressFill');
    const currentProgressLabelElem = document.getElementById('currentProgressLabel');
    const nextTierThresholdElem = document.getElementById('nextTierThreshold');
    const tierProgressMessageElem = document.getElementById('tierProgressMessage');

    if (tierProgressFillElem) tierProgressFillElem.style.width = `${progressPercent}%`;
    if (currentProgressLabelElem) currentProgressLabelElem.textContent = `$${usdFunded.toFixed(2)}`;
    if (nextTierThresholdElem) nextTierThresholdElem.textContent = `$${nextThreshold.toFixed(2)}`;

    if (tierProgressMessageElem) {
        if (currentTierIndex >= 3) {
            tierProgressMessageElem.innerHTML = '<strong>Maximum tier reached!</strong>';
        } else {
            const remaining = nextThreshold - usdFunded;
            tierProgressMessageElem.innerHTML = `Fund <strong>$${remaining.toFixed(2)} more</strong> to reach ${nextTierName} tier`;
        }
    }
...
function updateTierCards(currentTierIndex) {
    const bronzeCard = document.getElementById('bronzeTierCard');
    const silverCard = document.getElementById('silverTierCard');
    const goldCard = document.getElementById('goldTierCard');
    const bronzeStatus = document.getElementById('bronzeStatus');
    const silverStatus = document.getElementById('silverStatus');
    const goldStatus = document.getElementById('goldStatus');

    const tiers = [
        { card: bronzeCard, status: bronzeStatus, index: 1 },
        { card: silverCard, status: silverStatus, index: 2 },
        { card: goldCard, status: goldStatus, index: 3 }
    ];

    tiers.forEach(({ card, status, index }) => {
...
function updateActivitySummary(usdFunded) {
    ...
    const firstFundingDateElem = document.getElementById('firstFundingDate');
    const totalBonusEarnedElem = document.getElementById('totalBonusEarned');

    // These would need more complex tracking - for now show placeholder
    if (totalTransactionsElem) totalTransactionsElem.textContent = usdFunded > 0 ? '1+' : '0';
    if (firstFundingDateElem) firstFundingDateElem.textContent = usdFunded > 0 ? 'Recent' : '--';
    if (totalBonusEarnedElem) totalBonusEarnedElem.textContent = '0.00 $BRB';
}
